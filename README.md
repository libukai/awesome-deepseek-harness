...
- [dsh-agent-team](https://github.com/wowyuarm/dsh-agent-team)：给 DSH 一个可长期协作的持久 Agent 团队：Agent 是持久身份（私有 memory/notes/skills 跨会话保留），Workspace 按项目组织，Channel 由 Human 路由职责，Task Thread 把多个 session agent 串成一条推进线；成员会话超阈值自动压缩并提示先沉淀结论。MIT、npm / Release `0.1.7`（262+1 测试、真实浏览器验收），认证基线 DSH `0.1.2-rc.1`，双语 README，已收录于 awesome-dsh-plugin。单主机设计，不含分布式共识。
