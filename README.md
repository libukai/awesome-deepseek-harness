...
- [dsh-plugin-open-app](https://github.com/2nd1st/dsh-plugin-open-app)：把 open-mcp-apps 带进 DSH，每个 MCP app 都是侧边栏里自己的容器（独立 workspace + 会话 + App mode），带 agent 状态条、聊天内行内渲染与 App Store；MIT、npm `0.1.0`，但项目仍新，标注为早期。
