...
| [DeepSeekGUI](https://github.com/See-Sol-Lab/DeepSeekGUI) | Windows · Electron · 本地工作台（实验性 Linux AppImage） | `v1.1.0` 内置官方 DSH 运行时（0.1.2-rc.1），一键安装开箱即用，并与用户已装的 dsh 完全隔离——装上互不影响、卸载不留痕；对话旁提供改动 / Git / Worktree / 记忆四个本地面板，查看项目状态不消耗模型请求，Git / PR 专用工具全部经过官方审批弹窗，全局记忆由用户编辑、项目记忆由助手维护；跟随官方正式版持续更新 |
