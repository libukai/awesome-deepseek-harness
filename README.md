...
- [dsh-capability-panel](https://github.com/pure-craft/dsh-capability-panel)：面板展示当前 Agent 实际可触达的 Skill、工具与 MCP，并给出真实的在上下文状态（已加载 / 已截断 / 已挤出），支持按会话开关、按 preset 设默认能力、统计被拦截的调用次数，Skill / MCP 按来源分组并可一键打开来源目录；数据不出本机，仅提供回环 API。MIT、npm `1.1.0`，通过 `dsh plugin --profile web add dsh-capability-panel` 安装，需 DSH ≥ `0.1.2-alpha.4`，仓库含 466 个测试并设有 100% 覆盖率门禁。
