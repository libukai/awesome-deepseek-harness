...
- [dsh-locale-ru](https://github.com/warment/dsh-locale-ru)：为 DSH Web 增加俄语界面，覆盖 33 个命名空间、1061 条 UI 字符串（100% 覆盖），正式体（вы）与统一术语表；MIT、Release `v0.1.0`，插件只挂载客户端本地化模块，并通过配置层本地化权限预设显示名（只读 / 工作区写入 / 完全访问）；字典 CI 校验对 upstream（`check.mjs --strict`），上游新增字符串安全回退英文。
