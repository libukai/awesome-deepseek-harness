...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · 原生 AppKit / WebKit · Dock / 菜单栏 | `v1.0.1` 把官方本地 Web UI 的启动、已有浏览器标签复用、应用内窗口、服务控制和清晰的运行/停止指示合并为一个原生控制器；现有兼容 DSH 会原样复用，缺失时优先复用 Node.js 20+ 与 npm，只补齐隔离组件，全新 Mac 也可在无管理员权限下完成设置。Universal DMG 覆盖 Apple 芯片与 Intel 并附 SHA-256，但当前仅 ad-hoc 签名、尚未 Apple 公证；它是控制器 / 发行包，不是 `dsh plugin` |
