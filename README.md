...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · 原生 AppKit / WebKit · 通用包 | 面向官方本地 DSH Web UI 的轻量 Dock 与菜单栏控制器，支持保存浏览器 / 应用内打开方式、精确复用已有标签、服务健康与所有权保护，并提供可选的原生 WebKit 窗口；`v1.0.0` ZIP 同时支持 Intel 与 Apple 芯片，使用 ad-hoc 签名且尚未公证 |
