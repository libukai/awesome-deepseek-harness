...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · 原生 AppKit / WebKit · Dock / 菜单栏 | `v1.0.0` 把官方本地 Web UI 的启动、已有浏览器标签复用、应用内窗口和服务控制合并为一个原生控制器；全新 Mac 可一键安装当前用户专属的 Node.js 与已验证 DSH `0.1.1-rc.2`，并支持用户确认后检查 / 安装 npm `latest`。Universal ZIP 覆盖 Apple 芯片与 Intel 并附 SHA-256，但当前仅 ad-hoc 签名、尚未 Apple 公证；它是控制器 / 发行包，不是 `dsh plugin` |
