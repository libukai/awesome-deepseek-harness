...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · 原生 AppKit / WebKit · Dock / 菜单栏 | `v1.0.2` 把官方本地 Web UI 的启动、已有浏览器标签复用、应用内窗口、服务控制和清晰的运行/停止指示合并为一个原生控制器；现有兼容 DSH 会原样复用，缺失时优先复用 Node.js 22.19+ 与 npm，只补齐隔离组件。DSH 更新会先结合当前 Profile 验证启动与插件资源，失败不会替换可用环境。Universal DMG 覆盖 Apple 芯片与 Intel 并附 SHA-256，但当前仅 ad-hoc 签名、尚未 Apple 公证；它是控制器 / 发行包，不是 `dsh plugin` |
...
| [DeepSeek Harness for Windows](https://github.com/qingtan-labs/deepseek-harness-windows) | Windows 10 22H2+ · 原生 .NET Framework / WebView2 · x64 / ARM64 | `v1.0.0` 把官方本地 Web UI 面向当前用户的一键安装、已有浏览器标签复用、可选应用内窗口、托盘与服务控制合并为一个原生控制器；使用隔离且经过校验的 Node.js `22.21.1` 与 DSH `0.1.1-rc.2`，更新需要明确确认并支持回滚。ZIP 附 SHA-256，但尚无 Authenticode 签名；它是控制器 / 发行包，不是 `dsh plugin` |
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：在一个 DSH Bundle 中统一管理十个 IM 渠道与 AI Office，支持多机器人、流式回复、工作区 / 会话绑定、远程审批及超时任务结果补发。MIT、npm / Release `4.20.0`、Node.js 22.19+，Release Commit 的 CI 通过。新版兼容未修改的原版 DSH `0.1.5-alpha.1`，并继续覆盖此前四个原版 DSH Web profile；管理能力统一通过 Connection 的公开 `/api` Fetch 接口实现，不需修改或重编译 DSH。管理访问默认仅回环，`trusted-host` 仍受 DSH 浏览器认证及 Host / Origin 检查约束，更新和入站 TTL 管理始终仅允许回环。升级后需重启 Host 并刷新设置页；授权聊天用户仍可触发模型和工具，工作区 / 会话列表可能暴露本机路径和敏感元数据，应只向可信用户开放。
