...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · 原生 AppKit / WebKit · Dock / 菜单栏 | `v1.0.0` 把官方本地 Web UI 的启动、已有浏览器标签复用、应用内窗口、服务控制和用户确认的 npm `latest` 更新合并为一个原生控制器。安装时优先原样复用现有可运行的 DSH `0.1.1-rc.2` 或更高版本；需要回退时先复用 Node.js 20+ 与 npm，仅补装缺失的隔离组件且不覆盖外部环境或 `~/.dsh` 数据。单文件 Universal DMG 覆盖 Apple 芯片与 Intel 并附 SHA-256、源码 CI 已通过；当前仅 ad-hoc 签名、尚未 Apple 公证。它是控制器 / 发行包，不是 `dsh plugin` |
