...
- [dsh-im](https://github.com/xmanrui/dsh-im)：1 つの DSH Bundle で Feishu、WeChat、DingTalk、WeCom、QQ、Slack、Telegram、Discord、WhatsApp、AI Office を管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval を提供。MIT、npm / Release `4.13.0`、Node.js 22.19+ で、Release Commit の CI は成功。9 Channel は超時タスク結果の補送を共有する：Foreground Wait の Timeout 後も元の Harness Turn を追跡し、Host Restart / Reconnection 後も元の Chat / Thread へ Final Text または終了状態を送信する。明確な Send Failure は最大 3 回まで Retry、不確実な結果では自動 Retry を停止し、File、Question、Approval は再送しない。DingTalk と QQ に Session、Workspace、Model、Stop、Compaction の Menu を追加。WeChat は Proactive / Deferred Delivery に各受信者の緩存 `context_token` を使い、Upgrade 後は緩存を作るため 1 回の受信メッセージが必要で、iLink の Quota と Conversation Lifetime の制限は残る。Discord は Message Content Intent、Thread / Message / History Permission、File には Attach Files が必要。WhatsApp は既定で Self-chat。Secret / Token は Local Host のみに送られ管理 RPC は既定で Loopback だが、認可 Chat User は Model / Tool を起動でき、Workspace / Session List が Local Path や機微 Metadata を露出し得るため信頼 User のみに限定する。
