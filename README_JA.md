...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · ネイティブ AppKit / WebKit · Universal | 公式ローカル DSH Web UI 向けの軽量 Dock / メニューバーコントローラー。ブラウザ / アプリ内表示の既定値保存、既存タブの正確な再利用、サービスのヘルスと所有権保護、任意のネイティブ WebKit ウィンドウを備える。`v1.0.0` ZIP は Intel / Apple silicon の両方に対応し、ad-hoc 署名で未公証 |
