...
- [dsh-sonarqube](https://github.com/maxmilian/dsh-sonarqube)：SonarQube Community Build Web API 向けの読み取り専用 DSH プラグイン。インスタンス状態、ブランチや PR の品質ゲート、Issue と Security Hotspot の検索、個別ホットスポットの詳細、カバレッジ・重複・呼び出し側指定のメトリクスに対応。Issue とホットスポットの結果には正規化された位置情報（コンポーネント key、ファイルパス、行、テキスト範囲）が付く。MIT、npm `dsh-sonarqube` `0.1.0`、公式 `@deepseek-ai/*` は peerDependencies で宣言し `0.1.1-rc.2` を明示的に含む。0.1 のツールはすべて読み取り専用で、割り当て・確認・解決・再オープンは行わない。SonarQube の URL と対象プロジェクトにアクセスできるトークンが必要。Community Build `26.8.0.126808` で検証済みだが、同ビルドには SECURITY_HOTSPOT ルールが存在しないため、ホットスポット取得の成功経路はモックテストで担保。ツールのメタデータは英語・繁体中文・簡体中文・日本語に対応。
