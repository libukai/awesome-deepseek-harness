...
- [Busabase for DSH](https://github.com/busabase/busabase-dsh-plugin)：DSH を MCP 経由で、承認を前提とする Busabase のナレッジ／構造化データ基盤へ接続する。Agent は承認済みコンテンツの検索、Base / Record の操作、ChangeRequest の提出、会話内 Live Inspector での結果表示を行える。MIT、npm `0.1.5`、Node.js `>=24.18.0` が必要で、DSH の Peer 依存は `0.1.1-rc.2` に固定され、公開 commit の CI と Release workflow は成功済み。Local mode は Loopback の Busabase を必要時に起動または再利用し、Cloud mode は Browser OAuth を使って Token を DSH Credentials に保存する。MCP の権限上限は `changeRequest` に固定され、Agent は自身の提案を承認・Merge できない。現リリースでは Cloud Inspector の Review、Merge、Live refresh、埋め込み Preview は Local-only のため、Remote review には Busabase の Canonical link を使用する。
