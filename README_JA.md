...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · Native AppKit / WebKit · Dock / メニューバー | `v1.0.0` は公式ローカル Web UI の起動、既存 Browser Tab の再利用、App 内 Window、Service Control、User 確認後の npm `latest` Update を 1 つの Native Controller に統合。動作する DSH `0.1.1-rc.2` 以降があれば変更せず再利用し、Fallback が必要な場合も Node.js 20+ と npm を優先して不足する隔離 Component だけを追加し、外部 Runtime や `~/.dsh` Data を上書きしない。Apple Silicon / Intel 対応の単一 Universal DMG に SHA-256 を添付し Source CI も通過しているが、現在は ad-hoc 署名のみで Apple 公証は未実施。Controller / Distribution であり `dsh plugin` ではない |
