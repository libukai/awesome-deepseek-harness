...
- [dsh-forge](https://github.com/maxmilian/dsh-forge)：セルフホストの [Gitea](https://about.gitea.com/) / [Forgejo](https://forgejo.org/) に読み取り専用のコンテキストを提供する DSH プラグイン。インスタンスのバージョン、リポジトリ一覧、Issue と PR の検索・参照、PR の diff と変更ファイル、Actions の実行・ジョブ・プレーンテキストログに対応。MIT、Release `v0.3.1`、ビルド済み tarball を提供するためインストール時にローカルの TypeScript ビルドは不要。Gitea / Forgejo 共通の REST API を利用し、`@deepseek-ai/dsh-tools 0.1.1-rc.2` で検証済み、CI と実インスタンスの統合テストを備える。v0.3 のツールはすべて読み取り専用で、読み取り専用トークンの利用を推奨。ランタイムのメタデータは英語・繁体中文・簡体中文・日本語に対応。
