...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · ネイティブ AppKit / WebKit · Dock / メニューバー | `v1.0.1` は公式ローカル Web UI の起動、既存ブラウザータブの再利用、アプリ内ウィンドウ、サービス制御、明確な実行中 / 停止インジケーターを 1 つのネイティブコントローラーに統合する。互換性のある既存 DSH は変更せず再利用し、未導入の場合は Node.js 20+ と npm を優先的に再利用して不足する隔離コンポーネントだけを追加する一方、新規 Mac も管理者権限なしでセットアップできる。Universal DMG は Apple Silicon / Intel 対応で SHA-256 を添付するが、現時点では ad-hoc 署名のみで Apple 公証なし。`dsh plugin` ではなくコントローラー / ディストリビューションに分類される |
