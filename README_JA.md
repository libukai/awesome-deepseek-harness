...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · ネイティブ AppKit / WebKit · Dock / メニューバー | `v1.0.0` は公式 Local Web UI の起動、既存 Browser Tab の再利用、App 内 Window、Service Control を 1 つのネイティブ Controller に統合する。新規 Mac では User 専用 Node.js と検証済み DSH `0.1.1-rc.2` をワンクリック導入でき、User 確認後にのみ npm `latest` の確認 / 導入を行う。Universal ZIP は Apple Silicon / Intel 対応で SHA-256 を添付するが、現時点では ad-hoc 署名のみで Apple 公証なし。`dsh plugin` ではなく Controller / Distribution に分類される |
