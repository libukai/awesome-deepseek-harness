...
- [DSH Plugin Store](https://github.com/sandbaseai/dsh-plugin-store)：DSH Settings 内のネイティブマーケットプレイス。Community / Installed 表示、検索、タグ絞り込み、並べ替え、ローカル Profile へのインストールを提供。MIT、`0.1.0-preview.5`、DSH `0.1.0-rc.8` で検証済み。Catalog と Detail の Repository Identity が一致し、`verified` / `runtime_verified` の項目だけを対象に、検証済み Command から単一の npm package spec を抽出する。これは Provenance Gate であり、選択した Profile の依存解決や起動成功を保証しない。Transaction Snapshot と自動 Rollback は Roadmap 段階。
