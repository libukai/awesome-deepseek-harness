...
- [dsh-plugin-open-app](https://github.com/2nd1st/dsh-plugin-open-app)：open-mcp-apps を DSH に統合し、各 MCP アプリをサイドバー内の専用コンテナ（workspace + 会話 + App mode）にする。Agent ステータス行、チャット内インライン描画、App Store を備える。MIT、npm `0.1.0` だが新しいため初期段階。
