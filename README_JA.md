...
- [dsh-agent-team](https://github.com/wowyuarm/dsh-agent-team)：DSH に長期協働できる持続的 Agent チームを提供：Agent は永続的な識別（プライベートな memory/notes/skills がセッションをまたいで保持）で、Workspace がプロジェクトごとに整理し、Channel は Human が責務をルーティングし、Task Thread が複数の session agent を一本の進行ラインにつなぐ。メンバーのセッションはしきい値超過時に自動圧縮され、事前に結論の保存を促す。MIT、npm / Release `0.1.7`（262+1 テスト、実ブラウザ検収）、DSH `0.1.2-rc.1` 認定、二言語 README、awesome-dsh-plugin 収録。シングルホスト設計で分散合意は含まない。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：1 つの DSH Bundle で Feishu、WeChat、DingTalk、WeCom、QQ、Slack、Telegram、Discord、WhatsApp、AI Office を管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval を提供。MIT、npm / Release `4.9.0`、Node.js 22.19+ で、Release Commit の CI は成功。新版は 9 Channel に Workspace Alias を追加し、長時間 Wait を Activity ごとに更新し、Quoted Context から Internal ID と無関係な Metadata を除去する。Feishu Approval と Single-choice Question は既定で Actor-bound Interactive Card を使い、古い Card を拒否し Plain-text Fallback を保持する。`DSH_IM_INTERACTION_CARDS=0` で無効化できる。Discord は Message Content Intent、Thread / Message / History Permission、File には Attach Files が必要。WhatsApp は既定で Self-chat。Secret / Token は Local Host のみに送られ管理 RPC は既定で Loopback だが、認可 Chat User は Model / Tool を起動でき、Workspace / Session List が Local Path や機微 Metadata を露出し得るため信頼 User のみに限定する。
