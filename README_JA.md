...
現在確認できる公式 GitHub Developer Preview の最新版は [`0.1.2-rc.1`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.2-rc.1) で、npm `latest` と `next` はどちらも `0.1.2-rc.1` です。この RC は Full-history Turn Navigation、正確な Token / Duration 統計、Subagent Model 選択、ACP Standard Control、Schedule、Connection Retry を追加し、一方向の `report` を継続可能な双方向 `send_message` に置き換えます。Public WebFetch は SSRF 保護付きで既定 On、SQLite Session Backend は削除され（旧 Data は保持され Export に旧 Version が必要）、DeepSeek Adapter Request は有効な Plugin Package 名と Version を既定で付加します（無効化可能）。Session Log の Incremental Upload は Opt-in で既定 Off のままです。公式 Release は Security Audit 済みではなく、Sandbox、Approval、Permission が完全な隔離を保証しないとも明記しています。以下の DSH Version は作者が示す開発・テスト基準のみで、最新 Preview との互換性を自動的に意味しません。
...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · ネイティブ AppKit / WebKit · Dock / メニューバー | `v1.0.2` は公式ローカル Web UI の起動、既存ブラウザータブの再利用、アプリ内ウィンドウ、サービス制御、明確な実行中 / 停止インジケーターを 1 つのネイティブコントローラーに統合する。互換性のある既存 DSH は変更せず再利用し、未導入の場合は Node.js 22.19+ と npm を優先的に再利用して不足する隔離コンポーネントだけを追加する。DSH 更新は現在の Profile とプラグイン資産で事前検証され、失敗した候補が稼働中の環境を置き換えることはない。Universal DMG は Apple Silicon / Intel 対応で SHA-256 を添付するが、現時点では ad-hoc 署名のみで Apple 公証なし。`dsh plugin` ではなくコントローラー / ディストリビューションに分類される |
...
| [DeepSeek Harness for Windows](https://github.com/qingtan-labs/deepseek-harness-windows) | Windows 10 22H2+ · ネイティブ .NET Framework / WebView2 · x64 / ARM64 | `v1.0.0` は公式ローカル Web UI のワンクリック・ユーザー単位セットアップ、既存ブラウザータブの再利用、任意のアプリ内ウィンドウ、トレイとサービスの制御を 1 つのネイティブコントローラーに統合する。隔離・検証済みの Node.js `22.21.1` と DSH `0.1.1-rc.2` を使用し、更新には明示的な確認が必要でロールバックにも対応する。ZIP には SHA-256 を添付するが、現時点では Authenticode 署名なし。`dsh plugin` ではなくコントローラー / ディストリビューションに分類される |
...
- [dsh-whale-report](https://github.com/SenmuuuuW/dsh-whale-report)：Session Event Log から日次、週次、月次、年次、任意期間の Report を生成。MIT、Release / npm `0.6.1`、Node.js `^22.19 || >=24`、DSH Peer は `>=0.1.1-rc.2 <0.2.0`。0.6.0 は最初の厳密な Allowlist 付き Apply & Verify を追加し、Bash Timeout が繰り返された Evidence と User の明示承認がある場合だけ `shell.timeoutMs` を 60 秒から 120 秒へ提案・検証・監査・安全に Rollback できる。任意の Setting / Command、Auto-healing、Automatic Rollback は非対応。Release Commit の CI は成功し、Release は 393 Test と実 Package Acceptance を報告する。0.6.1 は過去価格の Effective Date、起動後の新規 Session、Resume Session の集計を修正し、Apply & Verify の境界は不変だが過去の Token / Cost 総額が実質的に変わる場合がある。新しく、制限付きとはいえ Setting 書き込み経路を持つため初期段階。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：10 IM Channel と AI Office を 1 つの DSH Bundle で管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval、超時 Task Result の補送を提供。MIT、npm / Release `4.20.0`、Node.js 22.19+ で Release Commit の CI は成功。未変更の公式 DSH `0.1.5-alpha.1` と既存の 4 DSH Web profile をサポートし、管理は DSH を変更・再構築せず Connection の公開 `/api` Fetch Interface で行う。Management は既定で Loopback-only、`trusted-host` は DSH Browser Authentication と Host / Origin Check に依存し、Update と Inbound-TTL Management は常に Loopback-only。Upgrade 後は Host Restart と Settings Refresh が必要。認可 Chat User は Model / Tool を起動でき、Workspace / Session List は Local Path や機微 Metadata を露出し得るため信頼 User のみに限定する。
...
- [DSH Plugin Upgrade Skill](https://github.com/oh-my-dsh/dsh-plugin-upgrade-skill)：DSH Plugin の Upgrade、Migration、Authoring、Testing、Release、Audit、Runtime Debug 向けの 8 Skills、`0.1.2-alpha.4` までの 45 Migration Card、12 General Strategy、29 Harbor Task、複数の Skill 有無比較評価を提供。MIT、`npx skills add oh-my-dsh/dsh-plugin-upgrade-skill` で導入でき、現在 Validation CI は成功。Unified Entry はまず Read-only Health Check を推奨し、Phase ごとに確認を分けるが、個別 Workflow は Source 編集、Dependency 導入、Test / Docker、Package、Publish を実行し得るため対応する承認が必要。2026 年 8 月 30 日作成、Release 未発行で、Migration Card も現行 `0.1.2-rc.1` 未対応のため初期段階。
