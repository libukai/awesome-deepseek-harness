...
- [Busabase for DSH](https://github.com/busabase/busabase-dsh-plugin): connects DSH over MCP to Busabase's approval-first knowledge and structured-data platform; Agents can search approved content, work with Bases and Records, submit ChangeRequests, and render results in a live in-conversation Inspector. MIT and npm `0.1.5`, requiring Node.js `>=24.18.0` with DSH peers pinned to `0.1.1-rc.2`; CI and Release workflows pass at the published commit. Local mode starts or reuses a loopback Busabase on demand, while Cloud mode uses browser OAuth and stores the token in DSH Credentials. The MCP permission ceiling is fixed at `changeRequest`, so an Agent cannot approve or merge its own proposals. Cloud Inspector review, merge, live refresh, and embedded previews remain Local-only in this release; remote review uses canonical Busabase links.
...
- [dsh-im](https://github.com/xmanrui/dsh-im): manages Feishu, WeChat, DingTalk, WeCom, QQ, Slack, Telegram, Discord, WhatsApp, and AI Office in one DSH Bundle, with multiple bots, streaming replies, workspace / Session binding, and remote approvals. It is MIT, npm / Release `4.13.0`, and Node.js 22.19+, with passing CI at the release commit. All nine channels now share deferred task delivery: after a foreground wait times out, it tracks the original Harness turn and sends final text or terminal status to the original chat or thread, including after Host restart or reconnection. Definite send failures retry at most three times, uncertain outcomes stop retrying, and files, questions, and approvals are never replayed. DingTalk and QQ add menus for Sessions, workspaces, models, stop, and compaction; WeChat reuses each recipient's cached `context_token` for proactive and deferred delivery, requires one inbound message after upgrade to populate that cache, and remains subject to iLink quotas and conversation lifetime. Discord still needs Message Content Intent and Thread / Message / History permissions, plus Attach Files for files; WhatsApp defaults to self-chat. Secrets and Tokens go only to the local Host and management RPC is loopback-only by default, but authorized chat users can still invoke models and tools, and workspace / Session listings may expose local paths and sensitive metadata, so access must remain limited to trusted users.
