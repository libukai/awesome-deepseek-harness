...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · Native AppKit / WebKit · Dock / menu bar | `v1.0.0` combines startup of the official local Web UI, existing-browser-tab reuse, an in-app window, service controls, and user-confirmed npm `latest` updates in one native controller. Setup preserves and reuses a working DSH `0.1.1-rc.2` or later; when a fallback is needed it reuses Node.js 20+ and npm first, adds only missing isolated components, and does not overwrite external runtimes or `~/.dsh` data. A single universal DMG covers Apple silicon and Intel with SHA-256 and passing source CI; it is currently ad-hoc signed and not Apple-notarized. This is a controller / distribution, not a `dsh plugin` |
