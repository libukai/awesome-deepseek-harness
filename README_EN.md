...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · Native AppKit / WebKit · Dock / menu bar | `v1.0.2` combines startup of the official local Web UI, existing-browser-tab reuse, an in-app window, and service controls with unambiguous running/stopped indicators in one native controller. A compatible existing DSH is reused unchanged; otherwise Node.js 22.19+ and npm are reused before only missing isolated components are added. DSH updates are staged and validated against the current Profile and plugin assets before activation, so a failed candidate does not replace a working environment. The universal Apple silicon / Intel DMG includes a SHA-256 checksum, but is currently ad-hoc signed and not Apple-notarized; it is a controller / distribution, not a `dsh plugin` |
...
| [DeepSeek Harness for Windows](https://github.com/qingtan-labs/deepseek-harness-windows) | Windows 10 22H2+ · Native .NET Framework / WebView2 · x64 / ARM64 | `v1.0.0` combines one-click per-user setup of the official local Web UI, existing-browser-tab reuse, an optional in-app window, tray controls, and service management in one native controller. It uses an isolated, verified Node.js `22.21.1` and DSH `0.1.1-rc.2`; updates require explicit confirmation and support rollback. The ZIP includes a SHA-256 checksum but is not yet Authenticode-signed; it is a controller / distribution, not a `dsh plugin` |
