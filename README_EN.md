...
- [deepseek-harness-genui](https://github.com/pengyue-polaron/deepseek-harness-genui): generates code-first, task-specific interfaces for Inline, Canvas, full-screen, and CLI localhost use; user interaction state returns to the task for later Agent turns, while MCP/API access requires user approval. MIT, npm `0.13.2`, supports DSH `^0.1.0-rc.6`, and is still new, so it is marked Early.
