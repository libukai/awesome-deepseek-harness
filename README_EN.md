...
- [dsh-forge](https://github.com/maxmilian/dsh-forge): Read-only DSH plugin for self-hosted [Gitea](https://about.gitea.com/) / [Forgejo](https://forgejo.org/), covering instance version, repository listing, issue and pull request search and read, PR diffs and changed files, plus Actions runs, jobs and plaintext logs. MIT, Release `v0.3.1`, with a prebuilt tarball so installation does not run a local TypeScript build. Built on the REST API shared by Gitea and Forgejo, tested against `@deepseek-ai/dsh-tools 0.1.1-rc.2`, with CI and live-instance integration tests. Every v0.3 tool stays read-only; a read-only token is recommended. Runtime metadata is available in English, Traditional Chinese, Simplified Chinese and Japanese.
