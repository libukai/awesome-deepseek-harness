...
- [dsh-plugin-open-app](https://github.com/2nd1st/dsh-plugin-open-app): brings open-mcp-apps into DSH, giving each MCP app its own sidebar container (workspace, session, and App mode), with an agent status strip, inline rendering in chat, and an App Store; MIT and npm `@2nd1st/dsh-plugin-open-app` `0.1.1`, installable with a single `dsh plugin add`, but still new and marked Early.
