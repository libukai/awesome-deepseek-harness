...
- [dsh-agent-team](https://github.com/wowyuarm/dsh-agent-team): a persistent agent team for long-running collaboration in DSH: agents are durable identities (private memory/notes/skills survive sessions), Workspaces organize per project, Channels route responsibilities from the Human, and Task Threads chain session agents into one line of progress; member sessions auto-compact past the threshold with a pre-compaction persistence hint. MIT, npm / Release `0.1.7` (262+1 tests, real browser acceptance), certified against DSH `0.1.2-rc.1`, bilingual README, listed on awesome-dsh-plugin. Single-host by design; no distributed consensus.
...
- [dsh-im](https://github.com/xmanrui/dsh-im): manages Feishu, WeChat, DingTalk, WeCom, QQ, Slack, Telegram, Discord, WhatsApp, and AI Office in one DSH Bundle, with multiple bots, streaming replies, workspace / Session binding, and remote approvals. It is MIT, npm / Release `4.9.0`, and Node.js 22.19+, with passing CI at the release commit. The new release adds workspace aliases across all nine channels, renews long-running waits on activity, and strips internal IDs and irrelevant metadata from quoted context. Feishu approvals and single-choice questions now use actor-bound interactive cards by default, reject stale cards, and retain a plain-text fallback; set `DSH_IM_INTERACTION_CARDS=0` to disable them. Discord still needs Message Content Intent and Thread / Message / History permissions, plus Attach Files for files; WhatsApp defaults to self-chat. Secrets and Tokens go only to the local Host and management RPC is loopback-only by default, but authorized chat users can still invoke models and tools, and workspace / Session listings may expose local paths and sensitive metadata, so access must remain limited to trusted users.
