...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · Native AppKit / WebKit · Dock / menu bar | `v1.0.0` combines startup of the official local Web UI, existing-browser-tab reuse, an in-app window, and service controls in one native controller. On a clean Mac it can install a private per-user Node.js plus the tested DSH `0.1.1-rc.2`, and can check / install npm `latest` only after user confirmation. The universal Apple silicon / Intel ZIP includes a SHA-256 checksum, but is currently ad-hoc signed and not Apple-notarized; it is a controller / distribution, not a `dsh plugin` |
