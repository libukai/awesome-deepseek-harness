...
| [DeepSeek Harness for macOS](https://github.com/qingtan-labs/deepseek-harness-macos) | macOS 13+ · Native AppKit / WebKit · Dock / menu bar | `v1.0.0` combines startup of the official local Web UI, existing-browser-tab reuse, an in-app window, and service controls in one native controller. A compatible existing DSH is reused unchanged; otherwise Node.js 20+ and npm are reused before only missing isolated components are added, while a clean Mac can still be set up without administrator access. The universal Apple silicon / Intel DMG includes a SHA-256 checksum, but is currently ad-hoc signed and not Apple-notarized; it is a controller / distribution, not a `dsh plugin` |
