...
- [DSH Plugin Store](https://github.com/sandbaseai/dsh-plugin-store): native marketplace inside DSH Settings with Community / Installed views, search, tag filters, sorting, and local Profile installation; MIT `0.1.0-preview.5`, verified against DSH `0.1.0-rc.8`. Installation is limited to repositories whose catalog and detail records agree and carry `verified` / `runtime_verified`, then extracts one npm package spec from the verified command. This is a provenance gate, not proof that the selected Profile resolves or boots; transactional snapshots and automatic rollback remain on the roadmap.
